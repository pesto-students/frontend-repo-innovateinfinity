...
import { getOrganizations, createStudent } from '../../../redux/actions/apiActions';
...
const AddStudent = () => {
    ...
    // useEffect(() => {
    //     if ((selectedOrganization.value !== null && selectedOrganization.value !== undefined) || profile === PROFILES.ORGANIZATION) {
    ...
    //             try {
    //                 const query = `?organizationId=${selectedOrganization.value}`;
    ...
    const handleOnSubmit = async (values) => {
        ...
        try {
            await createStudent(role, values);
            toast.success("Student added successfully");
            navigate("/students")
            dispatch(setGlobalLoader(false))
        } catch (error) {
            dispatch(setGlobalLoader(false))
            console.log("create student error - admin", error);
            toast.error(error.response.data.msg ?? "some error occured while creating student");
        }
    }
